...
public class QueryUser extends MyConnection{
	...
	public boolean createUser(Admin user) {
		try {
			...
			String query = "INSERT INTO `user`(id, lastName, firstName, login, password, adress, state, creatDate, updateDate, isSuperAdmin) VALUES (?,?,?,?,?,?,?,?,?)";
			...
			declaration.setString(4, user.getLogin());
